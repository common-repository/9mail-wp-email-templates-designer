...
class Enqueue {
	...
	public function register_scripts() {
		...
		foreach ( $lib_scripts as $script ) {
			wp_register_script( $this->slug . $script, EMTMPL_CONST['libs_url'] . $script . '.min.js', [ 'jquery' ], EMTMPL_CONST['version'], true );
		}
		...
		foreach ( $scripts as $script => $depend ) {
			wp_register_script( $this->slug . $script, EMTMPL_CONST['dist_url'] . $script . $suffix . '.js', $depend, EMTMPL_CONST['version'], true );
		}
	}
